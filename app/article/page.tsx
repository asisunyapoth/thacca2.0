...
export default function ArticlePage({ params }: { params: { page: string } }) {
  const currentPage = parseInt(params.page || '1', 10)

  // Pick relevant fields from blogs and sort by date
  let article = allArticles.map((item) =>
    pick(item, ['title', 'body', 'date', 'slug', 'description', 'image', 'tags'])
  )
  article = article.sort(sortByDate)

  const uniqueTags = extractUniqueTags(allArticles)
    ...
    <div className="lg:max-w-[90vw] mx-auto px-2">
      <div className="border-[0.5px] border-white">
        <div className='p-5 border-[0.5px] border-b-0 border-black text-center'>
          <h1 className='text-[42px] font-bold  text-black'>บทความ</h1>
        </div>
        {article.map((item, i) => (
